...
以下のURLに従い，Docker Rootless modeをセットアップする．<br/>
[Docker Rootless mode 導入方法](http://wiki.in.minelab/69acedda1864271665599d56)<br/>
sudo無しでコマンドが実行できることを確認する．
```sh
$ docker ps
...
<Callout type="error">
    以下の方法はrootとコンテナを共有してしまい，破壊的であるため非推奨．
### Docker セットアップ（旧）
<Callout type="default">
  環境構築を行うユーザで実行する．
</Callout>
...
<Callout type="warning">
    ポート番号を変更する場合は，[dashboard.py](/Scripts/dashboard/)内の使用ポートを書き換えること．
</Callout>
<Callout type="error">
	**以後の作業はコンテナ内で行うこと．**
</Callout>
...
### Tips
* aptのソースを以下の通り国内のミラーサーバに変更すると速度が上がる．
```yaml filename="/etc/apt/sources.list.d/ubuntu.sources"
Types: deb
...
Signed-By: /usr/share/keyrings/ubuntu-archive-keyring.gpg
```
* 次のWarningが発生する場合，NVIDIAの提供するDockerイメージにおいてKeyringの更新が追い付いていない．<br/>
手動で更新する．
```sh
W: https://developer.download.nvidia.com/compute/cuda/repos/ubuntu2404/x86_64/InRelease: Key is stored in legacy trusted.gpg keyring (/etc/apt/trusted.gpg), see the DEPRECATION section in apt-key(8) for details.
```
<Steps>
#### 以下のURLからgpgファイルをダウンロードする
https://developer.download.nvidia.com/compute/cuda/repos/
```sh
wget https://developer.download.nvidia.com/compute/cuda/repos/ubuntu2404/x86_64/cuda-archive-keyring.gpg
```
<Callout type="warning">
	R8現在Ubuntu24.04を利用しているが，都度，最新版を利用する．
</Callout>
#### gpgファイルを適当な場所に移動する 
```sh
mv cuda-archive-keyring.gpg /etc/apt/trusted.gpg.d/
```
#### sourcesリストを更新する
```sh
nano /etc/apt/sources.list.d/cuda.list
```
```yaml filename="/etc/apt/sources.list.d/cuda.list"
deb [signed-by=/etc/apt/trusted.gpg.d/cuda-archive-keyring.gpg] https://developer.download.nvidia.com/compute/cuda/repos/ubuntu2404/x86_64/ /
```
#### Warningが消えることを確認する
```sh
apt update
...
* `nvidia-smi`が通らない場合
	* 再起動してみる．
	* コンテナ外からも`nvidia-smi`が通らない場合．
		* http://wiki.in.minelab/651e1edbf179f6b61953aa90
		* https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html
		* ↑このへんを参考に頑張る．
	* コンテナ内からできない場合．<br/>
		* `--gpus all`オプションがついているか確かめる．
		* 適切なCountainer Imageを利用しているか確かめる．
		* `/etc/nvidia-container-runtime/config.toml`内の`no-cgroups = true`をfalseにする（コンテナ外）．
		```toml filename="/etc/nvidia-container-runtime/config.toml" {13}
...
## トラッキング・行動分類
任意のディレクトリに以下のDockerfile，compose.ymlを作成する
```yml filename="Dockerfile"
...
RUN apt update && \
    apt install -y software-properties-common sudo curl wget && \
    ...
    apt-fast upgrade -y && \
    apt-fast install -y \
    gcc g++ make cmake git ssh nasm \
    npm nodejs\
    python3.11-venv python3.11-dev pkg-config \
    libgl1-mesa-dev libx264-dev libx265-dev \
    ...
    entrypoint:
      ...
      - |
        ...
        if [ ! -s /root/.ssh/known_hosts ]; then
          ssh-keyscan -p 2224 git.in.minelab >> /root/.ssh/known_hosts 2>/dev/null
        fi

        export GIT_SSH_COMMAND="ssh -i /root/.ssh/id_ed25519 -o StrictHostKeyChecking=no"

        if [ ! -d ".git" ]; then
          echo "Cloning KptSORT..."
          git clone ssh://git@git.in.minelab:2224/nagai/KptSORT.git .
        fi

        if [ -d "tools" ]; then
          cd tools
          if [ ! -d "beehive-image-generator/.git" ]; then
            echo "Cloning beehive-image-generator..."
            git clone ssh://git@git.in.minelab:2224/nagai/beehive-image-generator.git
          fi
          cd ..
        fi

        if [ ! -d ".cv2" ]; then
          echo "Building OpenCV env..."
          cd installer/
          bash install_cv2_w_ffmpeg.sh
          cd /bee/KptSORT/
        fi

        ACTIVATE_FILE=".cv2/bin/activate"
        LD_PATH_LINE='export LD_LIBRARY_PATH=".cv2/lib:/usr/local/cuda/lib64"'
        if [ -f "$ACTIVATE_FILE" ]; then
          if ! grep -q "LD_LIBRARY_PATH" "$ACTIVATE_FILE"; then
            echo -e "\n# Added by Docker entrypoint\n$LD_PATH_LINE" >> "$ACTIVATE_FILE"
            echo "Updated $ACTIVATE_FILE with LD_LIBRARY_PATH"
          fi
        fi

        export LD_LIBRARY_PATH=".cv2/lib:/usr/local/cuda/lib64"
        exec /bin/bash
    tty: true
    stdin_open: true
...
<Callout type="error">
    ...
    ```sh filename="install_cv2_w_ffmpeg.sh" {8-10} showLineNumbers{53}
...
## DeepLabCut
### 以下に公式サイトの[Installation](https://deeplabcut.github.io/DeepLabCut/docs/installation.html)を引用する．
なお，本研究ではvenv環境で以下のバージョンを利用した．
* CUDA：12.9
* DeepLabCut：3.0.0rc13
* torch：2.11.0.dev20260212+cu129
* torchvision：0.26.0a0+326a11d
---
...
<Callout type="default">
##### Familiar with python packages and conda? Quick Install Guide:
...
![conda](https://images.squarespace-cdn.com/content/v1/57f6d51c9f74566f55ecf271/71e5d954-75a0-4534-9fa6-7ecc4bf1b76d/installDLC.png?format=1500w)
...
<Callout type="info">
  Download anaconda for your operating system: [anaconda.com/download/](https://www.anaconda.com/download/)
</Callout>
...
- TO DIRECTLY DOWNLOAD THE CONDA FILE conda:
  ...
  - click ➡️ for [CONDA FILE](https://github.com/DeepLabCut/DeepLabCut/blob/main/conda-environments/DEEPLABCUT.yaml#:~:text=Raw%20file%20content-,Download,-%E2%8C%98) and then click the "..." and select Download
    ![Screen Shot 2023-09-13 at 10 33 32 PM](https://github.com/DeepLabCut/DeepLabCut/assets/28102185/ec4295a5-e85c-4ce7-8c16-e6517a2cfa22)

-  **Now, in Terminal (or Anaconda Command Prompt for Windows users)**, if you clicked to download, go to your downloads folder.

<Callout type="warning">
  Windows users: **Be sure to open the program terminal/cmd/anaconda prompt with a RIGHT-click, "open as admin"** [2] ．
</Callout>
...
<Callout type="info">
  ...
  You can (on Windows) hold **SHIFT and right-click > Copy as path**, or (on Mac) right-click and while in the menu press the **OPTION key** to reveal Copy as Pathname [2] ．
</Callout>
...
<Callout type="warning" emoji="⚠️">
  **DeepLabCut TensorFlow Support** [2, 3] ．
  ...
  Installing TensorFlow and getting it to have access to the GPU can be a bit tricky. Check TensorFlow's [compatibility matrix](https://www.tensorflow.org/install/source#gpu) to know which version of CUDA and cuDNN you should install.
...
![](https://static1.squarespace.com/static/57f6d51c9f74566f55ecf271/t/5c3e46ca1ae6cfbb5c5d1ee0/1547585235033/cuda_driver.png?format=750w)
